...
async function findOrCreateGHLContact(data: {
  ...
  console.log(`[findOrCreateGHLContact] 🔍 Searching for contact by phone: ${data.phone}`);
  ...
  console.log(`[findOrCreateGHLContact] 📋 Phone search result:`, JSON.stringify(searchData).substring(0, 500));
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`[post-call-webhook] 📞 Fetching conversation ${conversation_id} from ElevenLabs...`);
    ...
    // DEBUG: Log full conversation metadata for troubleshooting
    console.log(`[post-call-webhook] 🔍 CONVERSATION METADATA:`, JSON.stringify({
      conversation_id,
      ...
      transcript_preview: conversationData.transcript?.slice(0, 3) || [],
      analysis_keys: Object.keys(conversationData.analysis || {}),
      analysis_summary: conversationData.analysis?.transcript_summary?.substring(0, 500),
      analysis_call_successful: conversationData.analysis?.call_successful,
      dynamic_variables: conversationData.metadata?.dynamic_variables || {},
      phone_external: conversationData.metadata?.phone_call?.external_number,
      started_at: conversationData.metadata?.started_at,
      ended_at: conversationData.metadata?.ended_at,
    }, null, 2));
    ...
    console.log(`[post-call-webhook] 📊 ANALYSIS RESULT: conversation=${conversation_id}, variant=${variantId}, outcome=${booking.outcome}, booked=${booking.booked}, selectedTime=${booking.selectedTime}, confirmedEmail=${booking.confirmedEmail}, confirmedName=${booking.confirmedName}`);
    ...
    if (booking.booked && GHL_API_KEY) {
      ...
      try {
        ...
        if (contactId && booking.selectedTime) {
          ...
          console.log('[post-call-webhook] ❌ GHL contact creation FAILED - no contactId returned');
