...
const healthHandler = (req: any, res: any) => res.json({ status: 'ok' });
...
// sms-status now combined into send-sms
...
app.all('/api/sms-status', (req, res) => { req.url = '/api/sms-status'; require('./api/send-sms').default(req, res); });
